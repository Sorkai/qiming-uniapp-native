...
// 风险等级标签
const riskLevelType = computed(() => (level: string) => {
  const map: Record<string, string> = {
    low: "success",
    medium: "warning",
    high: "danger",
...
const riskLevelText = computed(() => (level: string) => {
  const map: Record<string, string> = {
    low: "低风险",
    medium: "中风险",
    high: "高风险",
...
const fetchData = async () => {
  ...
  try {
    ...
    const { data } = await getTeacherDiscussions(params);
    discussions.value = data.list;
...
const fetchStats = async () => {
  try {
    const { data } = await getTeacherCourseStats();
    stats.value = data;
  } catch (error) {
...
const handleBatchApprove = async () => {
  ...
  try {
    ...
    const { data } = await batchReview({
      postIds: selectedIds.value,
      action: "approve"
    });
    ElMessage.success(`成功通过 ${data.success} 条，失败 ${data.failed} 条`);
    selectedIds.value = [];
    fetchData();
    fetchStats();
  } catch (error: any) {
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    const { data } = await batchDelete({ postIds: selectedIds.value });
    ElMessage.success(`成功删除 ${data.success} 条，失败 ${data.failed} 条`);
    selectedIds.value = [];
    fetchData();
    fetchStats();
  } catch (error: any) {
    ...
    <el-card shadow="never">
      ...
      >
        ...
        <el-table-column label="内容" min-width="300">
          <template #default="{ row }">
            <div class="post-content">
              <div class="post-title font-medium">
                <el-tag
                  v-if="row.isPinned"
                  size="small"
                  type="danger"
              ...
              <div class="post-excerpt text-gray-500 text-sm mt-1 truncate">
                {{ row.content.substring(0, 100) }}...
              </div>
              <div class="post-meta text-gray-400 text-xs mt-2">
        ...
        <el-table-column label="统计" width="120" align="center">
          <template #default="{ row }">
            <div class="flex justify-center gap-3 text-gray-500 text-sm">
              <span title="点赞">👍 {{ row.likeCount }}</span>
              <span title="回复">💬 {{ row.replyCount }}</span>
            </div>
          </template>
        </el-table-column>
