...
const html = existsSync(htmlPath)
  ? await import("node:fs").then(({ readFileSync }) => readFileSync(htmlPath, "utf8"))
  : "";
if (!html.includes("qiming-native-bridge.js")) {
  const patched = html.includes("</head>")
    ? html.replace("</head>", '  <script src="./qiming-native-bridge.js"></script>\n</head>')
    : `${html}\n<script src="./qiming-native-bridge.js"></script>\n`;
