...
- `student-course-study`: `/course/1?section=course-learn`
- `student-course-mastery`: `/course/1?section=mastery`
- `student-course-qa`: `/course/1?section=course-qa`
- `student-course-work`: `/course/1?section=homework-exam`
- `student-exams`: `/account?menu=exam-center`
- `student-exam-do`: `/student-exam-center/do/1`
- `student-exam-result`: `/exam-paper/result/1`
- `student-ai-app`: `/account/ai-app?mode=student`
- `teacher-dashboard`: `/welcome/index`
- `teacher-courses`: `/course/list`
- `teacher-course-category`: `/course/category`
- `teacher-course-assessment`: `/course/assessment`
- `teacher-discussion-review`: `/course/discussion/review`
- `teacher-ai-app`: `/ai-app/workspace`
- `admin-dashboard`: `/welcome/index`
- `admin-users`: `/user/list`
- `admin-course-category`: `/course/category`
- `admin-discussion-review`: `/course/discussion/review`

`summary.json` 会写入顶层 `totals.routes`、`totals.ok`、`totals.fail`，每条 route 结果也会包含
`ok` 与 `failReasons`，便于后续心跳脚本或 CI 直接判断验收状态。

## 线上 H5 发布

本地 DevTools 与 H5 smoke 可以使用 `http://localhost:8851`。真机预览、体验版和正式版必须使用
微信后台已配置的 HTTPS 业务域名，例如：

```bash
pnpm build
pnpm mini:preflight -- --dev-server https://aiedu.intelledu.cn --role teacher --entry /welcome/index
pnpm mini:preview -- --dev-server https://aiedu.intelledu.cn --role teacher --entry /welcome/index
```

发布前需要确认业务 H5 站点（当前为 `https://aiedu.intelledu.cn`）返回完整 H5 内容，并包含
`/static/js/` 入口脚本，而不是 0 字节空页。`https://aiedu-mp.intelledu.cn` 用于微信业务域名
校验和后台配置，不作为当前业务 H5 的承载站点；小程序上传、预览时的 `--dev-server` 应指向
真正承载前端页面的 HTTPS 站点。
