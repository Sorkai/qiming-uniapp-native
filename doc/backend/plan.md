...
所有接口都有共同的前缀：`edu/backend/v1`## 认证方式
...
- **接口路径**：`/course/generate/teacher/plan`- **请求方式**：POST
- **接口描述**：生成课程章节的教案
...
- **接口路径**：`/course/teacher/plan/list`- **请求方式**：GET
- **接口描述**：获取已生成的教案列表
...
- **接口路径**：`/course/teacher/plan/progress`- **请求方式**：GET
- **接口描述**：查询教案生成的进度状态
