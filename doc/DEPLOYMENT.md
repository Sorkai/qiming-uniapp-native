...
## 镜像命名与标签
- 镜像路径：`${CNB_DOCKER_REGISTRY}/${CNB_REPO_SLUG_LOWERCASE}/aiedu-api:<tag>`
- 常见标签：
  - `edge`（main）、`dev`（dev）、`pr-<num>-<sha>`（PR）、`branch-<name>-<sha>`（其他分支）
  - `vX.Y.Z` 与 `latest`（发布 Tag）

## 直接 Docker 运行
- 默认端口：`1004`
- 默认配置：镜像内置 `/app/etc/aieduapi.yaml`，来源仓库 `app/api/etc/aieduapi-docker.yaml`
